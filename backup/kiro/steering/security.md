...
- Use only tools in `security/shared/mcp-allowlist.json`. (The gate enforces this — see CLAUDE.md.)
- Never modify security mechanism or policy files, `.claude/settings.json`, Kiro hooks, or the protected screen modules. These are protected paths — the gate denies the write. If a task appears to need one, stop and emit a patch for a human.
