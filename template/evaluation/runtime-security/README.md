...
| [`classifier-selection.md`](classifier-selection.md) | Why this classifier, measured: 14/16 attacks, 3/8 legitimate withheld on the 24-case corpus the verdict was signed against; re-measured 2026-09-03 on 40 cases — 18/24 at the default window, 20/24 at 600/120, four misses all one family, plus the context-dilution and structured-output findings | questioning detection quality |
...
| `classifier-candidates/*.result.json` | Immutable benchmark output per candidate. `….result.json` is the 24-case run the verdict cites; `….corpus40.result.json` (default window) and `….corpus40.chunk600.result.json` (600/120) are the 2026-09-03 expansion — every result records its `chunk_policy` | `python3 Security-kit/eval/eval_runtime_injection.py --candidate-manifest … [--chunk-size 600 --chunk-overlap 120]` |
