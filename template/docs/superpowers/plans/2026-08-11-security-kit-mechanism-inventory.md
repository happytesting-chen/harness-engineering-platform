...
The other nine invariant-bearing tasks check *mechanisms* — deterministic code at a tool boundary. But the kit ships a **second kind of security component**: a skill that reasons. The runtime spec's four-zone table (`docs/superpowers/specs/2026-08-04-runtime-tool-mediation-design.md:999-1003`) names it **Zone 3 — drafting**: nondeterministic, human-present. Its rule is *"A model proposes; the human is the gate."* Zone 2 (`decide()`, the gates) is *"the only cell allowed to rule on a live request"*, and ☠ Zone 4 — a model deciding a live request unattended — is *"not a performance problem; it is an accountability problem."*
