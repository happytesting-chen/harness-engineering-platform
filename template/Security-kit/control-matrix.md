...
| `SEC-PROOF-GAP-001` | **GAP** — a *named* proof that disappears is silent. Measured 2026-08-16: `init.sh` names 9 of the 10 `tests/test_*.py` files individually and invokes no glob runner; `tests/test_mechanisms.py` (added this series, for the claims-register census) is the exception, so this increment's own census is reachable only from the CI pytest step, not from `./init.sh`. For the 9 that ARE named, each block is `if [ -f … ]` with **no `else`**, so renaming or deleting a test file removes its check without changing the build's verdict. The full-suite runner that would catch either gap lives in CI (`.github/workflows/harness-baseline.yml:77-101`), not in `init.sh`, deliberately: keeping `./init.sh` pytest-free is what makes the per-file wiring the zero-dependency path, and CI's pytest step is non-fatal when pytest is absent from the runner | `init.sh` block 5b names 9 of 10 tests individually; `[ -f ]` with no `else` | none — measured by hand | Fix: a required-set list covering all 10 files, so a missing one is an ERROR — Task 10 wires `tests/test_mechanisms.py` in and takes this to 10-of-10. Block `(b2)` does the no-`else` fix for `test_protected_paths.py` only; the other 9 (including the newly-added file) are still owed (§6.2 item 12(b)) |
