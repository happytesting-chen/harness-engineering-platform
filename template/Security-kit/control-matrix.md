...
| `SEC-PROOF-GAP-001` | **GAP** — a *named* proof that disappears is silent. Measured 2026-08-17: `init.sh` names 9 of the **11** `tests/test_*.py` files individually and invokes no glob runner. **Two** are unreached — `tests/test_mechanisms.py` (the claims-register census) and `tests/test_requirements.py` (the I6 spine, added in the requirements increment) — so both of this series' own proofs are reachable only from the CI pytest step, not from `./init.sh`. The earlier wording here said "9 of the 10 … is the exception", which was this row understating its own gap: correct arithmetic over a population that had since grown by one file. For the 9 that ARE named, each block is `if [ -f … ]` with **no `else`**, so renaming or deleting a test file removes its check without changing the build's verdict. The full-suite runner that would catch either gap lives in CI (`.github/workflows/harness-baseline.yml:77-101`), not in `init.sh`, deliberately: keeping `./init.sh` pytest-free is what makes the per-file wiring the zero-dependency path, and CI's pytest step is non-fatal when pytest is absent from the runner | `init.sh` block 5b names 9 of 11 tests individually; `[ -f ]` with no `else` | none — measured by hand | Fix: a required-set list covering all 11 files, so a missing one is an ERROR — wiring `tests/test_mechanisms.py` and `tests/test_requirements.py` in takes this to 11-of-11. Block `(b2)` does the no-`else` fix for `test_protected_paths.py` only; the other 8 are still owed (§6.2 item 12(b)) |
