...
| `SEC-RUNTIME-GAP-001` | **GAP** — everything above protects the agent that *builds* the product. A **deployed** agent has no hook system: nothing emits events, so the dispatcher is code you write and every tool call must route through it | design only — `Security-kit/runtime/` does not exist | none | `docs/superpowers/specs/2026-08-04-runtime-tool-mediation-design.md` |
