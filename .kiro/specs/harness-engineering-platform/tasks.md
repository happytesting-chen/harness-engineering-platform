...
- [x] 2. Implement Permission Gate (`governance/permission.py`)
  ...
  - [x]* 2.6 Write property tests for Permission Gate (Properties 1-7)
...
- [x] 3. Implement Audit Log (`observability/audit.py`)
  ...
  - [x]* 3.2 Write property tests for Audit Log (Properties 8-9)
...
- [x] 4. Implement Agent Loop (`demo/harness.py`)
  ...
  - [x]* 4.2 Write property tests for Agent Loop (Property 10)
...
- [x] 6. Create test infrastructure
  - [x] 6.1 Create `tests/fixtures.json` with generic ground-truth test cases
  ...
  - [x] 6.2 Implement `tests/test_fixtures.py` data-driven test runner
  ...
  - [x] 6.3 Implement `tests/test_e2e.py` Day 4 enforcement pattern
...
- [x] 7. Create CLAUDE.md template and context layer
  - [x] 7.1 Write `template/CLAUDE.md` with pre-filled generic content and placeholders
  ...
  - [x] 7.2 Create `template/context/README.md` explaining what domain documents belong in the context directory
  ...
  - [x] 7.3 Create `template/progress.md` with pre-filled session continuity structure
  ...
  - [x] 7.4 Create `template/session-handoff.md` template
    - Structure: Current Objective, Verification Evidence table, Files Changed, Decisions Made, Next Session Startup steps
    - _Requirements: 17.5_

- [x] 8. Create skill files and hook configuration
  - [x] 8.1 Create `.claude/commands/session-cycle.md` pre-filled generic skill
  ...
  - [x] 8.2 Create `.kiro/steering/session-cycle.md` with Kiro front-matter format
    - Same session-cycle content as Claude Code skill
    - Use `inclusion: auto` front-matter
    - _Requirements: 15.2, 18.4_

  - [x] 8.3 Create domain-specific skill placeholder file (`.claude/commands/domain-workflow.md`)
    - Structure provided, content marked with `{{DOMAIN_WORKFLOW_STEPS}}`, `{{DOMAIN_EXIT_CONDITION}}`
    - _Requirements: 15.3_

  - [x] 8.4 Create `.claude/settings.json` with base hook set (5 hooks)
...
- [x] 9. Implement `init.sh` verification script
  - [x] 9.1 Write `template/init.sh` with project detection and verification
...
- [x] 10. Checkpoint - Ensure template is complete and self-consistent
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. Create `tools/tool-registry.md` and template README
  - [x] 11.1 Create `template/tools/tool-registry.md` documenting tool vetting process
    - Include fields: tool name, version pin, review status, approval rationale
    - _Requirements: 13.3_

  - [x] 11.2 Create template README documenting file classification and tool compatibility
...
- [x] 12. Implement Demo Script (`demo/demo.py`)
  - [x] 12.1 Implement `demo.py` with scripted 5-turn enforcement demonstration
...
- [x] 13. Create Red Team Harness example (`examples/red-team-harness/`)
  - [x] 13.1 Create Red Team example directory as filled copy of template
  ...
  - [x] 13.2 Create Red Team CLAUDE.md and context documents
  ...
  - [x] 13.3 Create Red Team policy files
  ...
  - [x] 13.4 Extend `tests/fixtures.json` with Red Team domain-specific test cases
  ...
  - [x] 13.5 Create Red Team demo script (`examples/red-team-harness/demo/demo.py`)
